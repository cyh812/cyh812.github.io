...
function newSystem(){
  ...
  for(j=0;j<edgelist.length;j++){
    if(!useRand || rand < randFloat){ //这个判断条件等于没有
        ...
        if(nodes[edgelist[j].source] != undefined && nodes[edgelist[j].target]!= undefined && points.length > 0){
          ...
          var c;
          if(stype != ttype){
            c = new THREE.Color(0x2F4F4F); // 如果两者学科不一样，则取白色
          }else{
            c = new THREE.Color(0x2F4F4F); // 如果一样，取对应学科的颜色
          }
  ...
  var lineMaterial = new THREE.LineBasicMaterial({vertexColors: THREE.VertexColors, transparent: true, opacity: 0});
