...
function search(string){
  ...
  for(n in nodes){  // n代表论文索引
    ...
    if(nodes[n].title.toLowerCase().includes(string)){
      ...
      list+="<li><a href=\javascript:zoomTo('"+n.toString()+"')\ data-track'click' data-track-action='view search result' data-track-category='info' data-track-label='button'><img class='icon-small' src='textures/big.svg'/ alt='view result'></a><a href=\javascript:updateInfo('"+n+"','"+c+"')\ data-track'click' data-track-action='expand search result'  data-track-category='info' data-track-label='button'><img class='icon-small' src='textures/hua.svg' alt='expand result'/></a><a href=\javascript:zoomTo('"+n.toString()+"')\ data-track'click' data-track-action='view search result'  data-track-category='info' data-track-label='button'><p>"+nodes[n].title+"</p></a></li>"
      found = true;
    }
  }
  if(found){
...
function updateInfo(index, color, node){
  if(node == undefined){
    node = nodes[index.trim()]; //trim() 方法用于删除字符串的头尾空白符，空白符包括：空格、制表符 tab、换行符等其他空白符等。
  }
  $('#info').empty();
  ...
  // 添加标题，分类，年代和链接
  // $('#info').append("<h1>"+node.title+"</h1><h2><span style='color:#"+color+"'>"+node.type+"</span> "+node.year+"</h2><a href=\"#\" onclick=\"change('"+index+"')\" ><img src='textures/跳转.svg' alt='open paper' class='icon'/></a>").fadeIn('slow');
  $('#info').append("<h1>"+node.title+"</h1><h2><span style='color:#"+color+"'>"+node.type+"</span> "+node.year+"</h2><a href='artwork.html?a="+index+"' target='_blank' ><img src='textures/跳转.svg' alt='open paper' class='icon'/></a>").fadeIn('slow');
  ...
  if(edges.length > 0){
    ...
    for(l=0;l<edges.length;l++){
      ...
      list+="<li><a href=\javascript:zoomTo('"+zid.toString()+"')\ data-track'click' data-track-action='view search result'  data-track-category='info' data-track-label='button' ><img class='icon-small' src='textures/big.svg'/ alt='view result'></a><a href=\javascript:updateInfo('"+zid+"','"+color+"')\ data-track'click' data-track-action='expand search result'  data-track-category='info' data-track-label='button'><img class='icon-small' src='textures/hua.svg' alt='expand result'/></a><a data-track='click' data-track-action='zoom to node "+zid+"'  data-track-category='info' data-track-label='button' href=\javascript:zoomTo('"+zid+"')\><p>"+ttl+"</p></a></li>"
    }
    list+="</ul>"
    $('#info').append(list);
